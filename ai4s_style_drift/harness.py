...
def build_harness_state(report: dict[str, Any], prompt: str = "") -> dict[str, Any]:
    ...
    deepseek = report.get("deepseek_report", {"status": "not_requested"})
    reporter_status = "completed" if deepseek.get("status") in {"completed", "disabled", "not_requested"} else "completed_with_warning"
    reporter_summary = "写入结论、边界、局限与复现成本"
    if deepseek.get("status") == "completed":
        reporter_summary += "；DeepSeek 已生成审计版叙述"
    elif deepseek.get("status") == "disabled":
        reporter_summary += "；DeepSeek 未配置，使用本地确定性报告"
    elif deepseek.get("status") == "error":
        reporter_summary += "；DeepSeek 调用失败，保留本地报告"
    stages = [
        ...
        {"id": "reporter", "status": reporter_status, "title": "报告输入员", "summary": reporter_summary, "evidence_ids": ["CONCLUSION", *[f"LIMIT-{i}" for i, _ in enumerate(report["limitations"], 1)]]},
    ]
    return {"prompt": prompt, "roles": ROLE_META, "stages": stages, "status": "completed", "output": {"title": report["title"], "conclusion": report["conclusion"], "failed_hypotheses": report["failed_hypotheses"], "reproduction_cost": report["reproduction_cost"], "deepseek_status": deepseek.get("status")}}
